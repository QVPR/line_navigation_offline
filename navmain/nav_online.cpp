...
void imageCallback(const sensor_msgs::Image::ConstPtr &msg)
{
	...
	if (!initialised)
	{
		std::cout << "init localisation" << std::endl;
	...
	std::cout << "publish cmd_vel " << v << " " << w << std::endl;
	geometry_msgs::Twist cmd_vel = geometry_msgs::Twist();
	cmd_vel.linear.x = v;
	cmd_vel.angular.z = w;
	twistPub.publish(cmd_vel);
...
int main(int argc, char **argv)
{
	...
	n.param("display_image", display_image, false);
	...
	/* Setup image display */
	std::cout << "setup image display " << display_image << std::endl;
	dispNav *d;
	d = new dispNav;
	d->saveimage(false);
	d->displayimage(display_image);
	d->setDisptime(100);
	...
	/* setup navigation */
	std::cout << "setup navigation" << std::endl;
	nav = navigation(d, K);
